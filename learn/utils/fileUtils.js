...
export function formatDatetime(date) {
  ...
  return localDatetime
    ...
    .slice(0, 19)
    .replace(/-/g, "/")
    .replace("T", " at "); // ISO format and return local datetime
}

export function getIconByMIMEType(fileType, isFolder) {
  // Returns a font awesome icon name for a specified file type
  const icons = [
    // Array of MIME type prefixes and their associated file icons. The first match is used, so place more specific types higher.
  ...
  return isFolder
    ? "folder"
    : (icons.find(([mimeType]) => fileType.startsWith(mimeType)) || [
        "unknown",
        "file",
      ])[1];
}
